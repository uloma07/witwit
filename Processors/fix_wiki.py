...
def process_record(args):
    ...
    # Assuming there's a unique identifier field called '_id' or any unique field
    unique_id = data['_id']

    # Check if the record exists in either collection B or collection C
    if indoor_collection.find_one({'_id': unique_id}) or outdoor_collection.find_one({'_id': unique_id}):
        # If found, update the record in collection A with a new field 'processed' set to True
        source_collection.update_one({'_id': unique_id}, {'$set': {'processed': True}})
        print(f'Record {unique_id} updated in {source_name} as processed.')
    else:
        try:
            url = data['url']
